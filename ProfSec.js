...
async function telebot() {
  ...
  task.addEventListener("reward", (event) => {
    ...
    async function AdsGramReward() {
      ...
      // Reward is now 1 ticket (used for the lucky wheel) instead of coins.
      fpAddTickets(1);
      fpIncrementDailyCount();
      Swal.fire({
        title: "Earned 1 ticket!",
        icon: "success",
        draggable: true
      });
  ...
  if (ReffIdAcountFounder) {
    ...
    // do nothing or whatever you want
        ...
        setTimeout(() => {
          ...
          PInfo.innerText = `Tap START to earn more Coin!`;
        ...
        document.getElementById('ClaimFriendGift').onclick = (() => {
          document.getElementById('AdsLoadingPage1').style.display = 'flex';
          if (GameStartReady == true) {
            console.log('Play Game');
            GameStartReady = false;
            document.getElementById('FriendInvateId').style.display = 'none';
            console.log('Showing ads...');
            document.getElementById('tads-container-9486').style.display = 'flex';
            const adsNotFoundCallback = () => {
              document.getElementById('tads-container-9486').style.display = "none";
              ...
              // --------------------------FreePay Ads Loading
              const AdPageLoadingPage = document.getElementById('FreePayLoadingAdsLoadinPage');
              AdPageLoadingPage.style.display = 'Flex';
              async function FreePayAds() {

                // -----------------Onclick on FreePay Ads
                function OnClickRewardFreePay() {
                  AdPageLoadingPage.style.display = 'none';
                  document.getElementById('AdsLoadingPage1').style.display = 'none';
                ...
                let { data: adsinfo1, error1 } = await _supabase
                ...
                let { data: AdViewer1, error } = await _supabase
                  .from('AdViewer')
                  .select('AdId')
                  .eq("LastViewer", a)
                ...
                const AllAds1 = adsinfo1;
                const AdViewed1 = AdViewer1;
                const AdsShowded1 = AdViewed1.map(item => item.AdId);
                const AdsOkToShow1 = AllAds1.filter(item => !AdsShowded1.includes(item.id));
                const AdReady1 = AdsOkToShow1[0];

                if (!AdReady1) {
                  // console.log('Ads Cant Found');
                  betButton.style.pointerEvents = "all";

                  AdPageLoadingPage.innerHTML = `
                    ...
                    <p class="LoadingAdPGolden"  id="AdminLink" style="background-color: #fff;padding: 0.5rem;border-radius: 0.5rem;">Ads By FreePay</p>
                  `;
                  document.getElementById('AdminLink').addEventListener('click',()=>{
                    window.location.href = 'https://t.me/TheFreePayBot_Admin';
                  })
                  setTimeout(() => {
                    AdPageLoadingPage.style.display = 'none';
                  }, 3000)
  ...
  const targetDate = new Date('2026-11-17').getTime();
  setInterval(() => {
    const now = Date.now();
...
window.addEventListener('load', () => {
  Profbtn.className = 'x fp-tab-btn fp-tab-active';
  Taskinput.className = 'x fp-tab-btn';
  shopbtn.className = 'fp-tab-btn';
...
shopbtn.addEventListener('click', () => {
  Profbtn.className = 'x fp-tab-btn';
  shopbtn.className = 'fp-tab-btn fp-tab-active';
  Taskinput.className = 'x fp-tab-btn';
...
document.getElementById('Taskinput').addEventListener('click', () => {
  Taskinput.className = 'x fp-tab-btn fp-tab-active';
  Profbtn.className = 'x fp-tab-btn';
  shopbtn.className = 'fp-tab-btn';
  // earnbtn.className = '';
...
document.getElementById('Shopinput').innerHTML = `<svg class="fp-tab-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 8h12l-1 12a2 2 0 0 1-2 2H9a2 2 0 0 1-2-2L6 8z" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><path d="M9 8V6a3 3 0 0 1 6 0v2" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg><span class="fp-tab-main">Shop</span>`;
...
// NFTs footer tab removed — "NFTs Market" is now reachable from inside the Shop tab (see freepay-addons.js)
