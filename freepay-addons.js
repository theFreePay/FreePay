...
/* ---------------- AdsGram go-button press feedback ---------- */
/* Card pulses gently while the task is available ("go"). As soon
   as the go button is tapped, the card stops pulsing and dims to
   a grayish "in progress / disabled" look. If the ad SDK reports
   an error or no banner was found, the card lights back up so the
   user can try again. */
function fpSetupTaskPressFeedback() {
  document.querySelectorAll('.taskGram, .taskGram2').forEach((task) => {
    const card = task.closest('.Tasks');
    if (!card) return;

    task.addEventListener('click', (e) => {
      const target = e.target && e.target.closest ? e.target.closest('[slot="button"]') : null;
      if (target) card.classList.add('fp-task-pressed');
    });

    ['onError', 'onBannerNotFound'].forEach((evt) => {
      task.addEventListener(evt, () => card.classList.remove('fp-task-pressed'));
    });
  });
}
